...
export const repositories: RepositoryConfig[] = rawRepositories.map(repo => {
  ...
  return {
    ...
    owner,
    repo: repoName,
    displayName: repoName.split('-').map(word => 
      word.charAt(0).toUpperCase() + word.slice(1)
    ).join(' '),
  };
});

export function getRepositoryBySlug(slug: string): RepositoryConfig | undefined {
...
export function getVersionBySlug(repo: RepositoryConfig, versionSlug?: string): VersionConfig {
  if (!versionSlug) {
    return repo.versions.find(v => v.version === repo.latest_version) || repo.versions[0];
  }
  return repo.versions.find(v => v.version === versionSlug) || repo.versions[0];
}

export function getBaseFileName(file: LimitedFile|string): string | undefined {
  let filename = "";
